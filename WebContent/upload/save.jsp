...
%>
<%
	//넘겨받은 파라미터 중 파일에 대한 저장처리!!(파일 업로드!!)
	String saveDir=null;

	//웹서버상의 경로를 하드경로로 지정하지 말고, 
	//프로그래밍적으로 추출한 후 이용하자!!
	//이유? 경로를 개발자가 결정짖어 놓으면, 서버의 환경이 바뀔경우에 에러발생할 수 있음..
	//(윈도우 --> 리눅스)
	//jsp 내장 객체중 웹어플리케이션의 전역적 정보를 가진 객체를 이용하여 경로를 추출해낼수 있다..
	...
	try{
		...
		//컴포넌트 객체를 이용해서 받아야한다
		//따라서 MultipartRequest 객체의 메서드 중 getParameter()
		//메서드를 이용하면 된다
		...
		e.printStackTrace();
		out.print("<script>");
		out.print("alert('파일의 용량은 1kbyte를 넘을 수 없습니다');");
